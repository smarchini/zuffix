...
template <typename T, template <typename U> class RH> class SignatureZuffixArray {
  ...
  public:
	...
	SignatureZuffixArray(String<T> string) : text(std::move(string)), sa(SAConstructByGrebnovSAIS(text)), lcp(LCPConstructByKarkkainenPsi(text, sa)), ct(CTConstructByAbouelhoda(lcp)), hash(&text) {
  ...
  private:
	...
	friend std::ostream &operator<<(std::ostream &os, const SignatureZuffixArray<T, RH> &ds) { return os << ds.text << ds.sa << ds.lcp << ds.ct << ds.z << ds.maxhlen; }
	friend std::istream &operator>>(std::istream &is, SignatureZuffixArray<T, RH> &ds) { return is >> ds.text >> ds.sa >> ds.lcp >> ds.ct >> ds.z >> ds.maxhlen; }
