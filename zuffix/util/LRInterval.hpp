 ...
 */
template <class T> class LRInterval {
  public:
	const T from, to;

	explicit LRInterval(T point) : LRInterval(point, point) {}
	LRInterval(T from, T to) : from(std::move(from)), to(std::move(to)) {}
	 ...
	 * @return an interval
	 */
	static LRInterval<T> empty() { return LRInterval(1, 0); }
	 ...
	 */
	bool operator==(const LRInterval<T> &oth) const {
	 ...
	 */
	bool operator!=(const LRInterval<T> &oth) const { return !(this == oth); }

	/** Serialize the interval in a human-readable format */
	friend std::ostream &operator<<(std::ostream &os, const LRInterval<T> &interval) {
		if (interval.isEmpty()) return os << "[]";
		if (interval.isSingleton()) return os << "[" << interval.from << "]";
