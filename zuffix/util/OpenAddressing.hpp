...
public:
  ...
  void store(uint64_t signature, T value) {
    ...
    while (std::get<0>(table[pos]) != signature &&
           std::get<0>(table[pos]) != 0) {
#ifdef DEBUG_STATS
      store_duplicate++;
#endif
  ...
  std::optional<T> operator[](uint64_t signature) {
    ...
    if (std::get<0>(table[pos]) == signature)
      ...
      get_null++;
#endif
    return std::get<0>(table[pos]) == signature
               ? std::optional(std::get<1>(table[pos]))
               : std::nullopt;
  };
...
private:
  friend std::ostream &operator<<(std::ostream &os,
                                  const OpenAddressing<T, AT> &ds) {
    return os << ds.table;
  }
  friend std::istream &operator>>(std::istream &is, OpenAddressing<T, AT> &ds) {
    return is >> ds.table;
  }
