...
template <typename T, hash_function Hash> class ZuffixArray {
  ...
  public:
	...
	// Enhanced Zuffix Array
	ZuffixArray(String<T> str, uint64_t seed) : string(std::move(str)), sa(buildSuffixArray(string)), lcp(buildLCP(string, sa)), up(string.length()), down(string.length()), next(string.length()) {
		...
		for (size_t i = 1; i < string.length(); i++) {
			while (lcp[i] < lcp[stack.top()]) stack.pop();
	...
	void visitPre(size_t i, size_t j, size_t name_len, size_t d) {
		...
		assert((i == 0 && j == string.length() - 1 && name_len == 0) || ((i != 0 || j != string.length() - 1) && name_len == 1 + max(lcp[i], lcp[j + 1])));
		...
		if (i == j) {
			...
			if (name_len != extent_len) {
				...
				for (size_t k = sa[i]; k < sa[i] + handle_len; k++) cout << string[k];
			}
			...
			for (size_t p = 0; p < children.size() - 1; p++) visitPre(children[p], children[p + 1] - 1, extent_len + 1, d + 1);
	...
	size_t fatBinarySearch(const String<T> &v, int64_t from, int64_t to) const {
		...
		for (size_t i = 0; i < v.length(); i++) std::cout << v[i] << " ";
...
#ifdef DEBUG
		std::cout << "Final interval: (" << from << ".." << (to + 1) << "); top: [" << interval(top).first << ", " << interval(top).second << "]";
#endif
		return top;
	}

	LRInterval<size_t> findExact(const String<T> &v) const {
		size_t left = 0;
		...
		for (size_t i = 0;; i++) {
			...
			while (i < limit && v[i] == string[sa[left] + i]) i++;

			if (i == m) return LRInterval(left, right);
			if (i < extent_len) return LRInterval<size_t>::empty();

			assert(i == extent_len && "i != extent_len");
			assert(m > extent_len && "m > extent_len");

			if (left == right) return LRInterval<size_t>::empty();

			auto children = getChildren(left, right);

			size_t c;
			for (c = 0; c < children.size(); c++) {
				if (c == children.size() - 1 && children[c] == right && extent_len + 1 == string.length() - sa[children[c]]) {
#ifdef DEBUG
			...
			if (c == children.size()) return LRInterval<size_t>::empty();
	...
	LRInterval<size_t> find(const String<T> &v) const {
		size_t m = v.length();
		...
		if (i == m) return LRInterval(child_left, child_right);
		if (i < m) return LRInterval<size_t>::empty();
  ...
  private:
	...
	static Vector<size_t> buildLCP(const String<T> &string, const Vector<size_t> &sa) {
		...
		for (size_t i = 1; i < length; i++) {
			while (sa[i - 1] + lcp[i] < length - 1 && sa[i] + lcp[i] < length - 1 && string[sa[i - 1] + lcp[i]] == string[sa[i] + lcp[i]]) lcp[i]++;
		}
	...
	void buildZMap(size_t i, size_t j, size_t name_len) {
		...
		assert((i == 0 && j == string.length() - 1 && name_len == 0) || ((i != 0 || j != string.length() - 1) && name_len == 1 + std::max(lcp[i], lcp[j + 1])));
		...
		for (size_t p = 0; p < children.size() - 1; p++) buildZMap(children[p], children[p + 1] - 1, extent_len + 1);

		buildZMap(children[children.size() - 1], j, extent_len + 1);
	}

	static LRInterval<size_t> interval(size_t pos) { return LRInterval(pos >> 32, pos); }
